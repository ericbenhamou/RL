...
class Rl_linear(Rl_Method):
    def __init__(self, r_t, N, M, method_type, alpha, gamma, random_init, squashing_dim=2):
...
class Rl_full_matrix(Rl_Method):
    def __init__(self, r_t, N, M, method_type, alpha, gamma, random_init, mean, sigma, squashing_dim=2):
        if method_type != 'Q-Learning' and method_type != 'SARSA':
             raise ValueError('invalid algorithm: supported Q-Learning and SARSA')
        self.method_type = method_type
        self.N = N
